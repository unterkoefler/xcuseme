...
import Date exposing (Unit(..), Date)
...
toStringHelp { today, other } =
    let
        weekday = Date.format "EEEE" other
        daysAgo = Date.diff Days other today
        weeksAgo = Date.diff Weeks other today
        monthsAgo = Date.diff Months other today
    in
    if daysAgo < 0 then
