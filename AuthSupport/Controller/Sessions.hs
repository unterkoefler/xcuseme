...
-}
module AuthSupport.Controller.Sessions
( newSessionAction
, createSessionAction
, deleteSessionAction
...
import Data.Aeson
import Network.HTTP.Types (status401, status201)
    ...
    >>= \case
        Just (user :: record) -> do
            ...
            when isLocked do
                ...
                renderJsonWithStatusCode status401 (Data.Aeson.object [ "success" .= False, "errorMessage" .= ("User is locked" :: Text) ])
            ...
            if verifyPassword user (param @Text "password")
                then do
                    ...
                    user <- user
                            ...
                            |> updateRecord
                    renderJsonWithStatusCode status201 (Data.Aeson.object [ "success" .= True ])
                    ...
                    when (get #failedLoginAttempts user >= maxFailedLoginAttempts user) do
                        Lockable.lock user
                        pure ()
                    renderJsonWithStatusCode status401 (Data.Aeson.object [ "success" .= False, "errorMessage" .= ("Invalid Credentials" :: Text) ])
        Nothing -> do
            setErrorMessage "Invalid Credentials"
            renderJsonWithStatusCode status401 (Data.Aeson.object [ "success" .= False, "errorMessage" .= ("Invalid Credentials" :: Text) ])
